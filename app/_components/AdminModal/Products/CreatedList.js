...
export default function CreatedList({
  ...
  handleSelectProduct,
  productGalleries,
  handleSave, // добавляем как пропс
}) {
  return (
    <div className='w-[500px] h-full relative flex flex-col'>
      <div className='h-screen w-[380px] fixed inset-0 bg-snowy flex flex-col justify-between overflow-y-scroll no-scrollbar'>
              ...
              createdList.map((item) => (
                <div key={item.id} onClick={() => handleSelectProduct(item.id)}>
                  <CreatedListItem
                    ...
                    item={item}
                    productGallery={productGalleries[item.id] || []}
                  />
                </div>
              ))
            }
        ...
        <div className='w-full bg-white flex px-6 py-4 items-center justify-between gap-2'>
          <button onClick={createNewProduct} className='py-3 flex items-center px-4 border rounded-xl text-greenView'>
